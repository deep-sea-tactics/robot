...
currentLimit=20  # marqArr current (fuse) minus current for electronics (cameras,pi,arm)

class dynamicpoweralloc(orders,motorPins)
	def calcLimit(x):
	...
	def powerRequests(powerRequestArray,debug=False):
		rqArr = np.array(powerRequestArray)*100 # simplifies equation entry: make sure that incoming values are decimal out of 1
		requestAdjusted = calcLimit(rqArr) # polynomial in terms of rqArr: THIS CURVE CALCULATED FOR BLUEROBOTICS T200 out of 100
		fullPowerAdjusted = calcLimit(100) # when rqArr in requestAdjusted = 100 
		powerAmpsOut = np.around((((requestAdjusted)**2)*currentLimit)/(sum(requestAdjusted)*(fullPowerAdjusted)),6)
		return(powerAmpsOut)
		if debug == True:	
...
dynamicpoweralloc.ampsToPWMEQ([15,0,15],[1,0,-1])	
dynamicpoweralloc.powerRequests(powerRequestArray,True) #temp,debug
