...
export function move(direction: vector.Vector, torque: vector.Vector): MoveOutput {
	...
	const outputs = gradientDescent(
		(a, b, c, d, e, g) => {
			...
			const directionDifference = vector.magnitude(
				vector.subtract(direction)(calculateForce(motorMovement))
			);
			const torqueDifference = vector.magnitude(
				vector.subtract(torque)(calculateTorque(motorMovement))
			);
