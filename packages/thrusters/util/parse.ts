...
const tupledData: Record<number, [pwm: number, current: number, force: number][]> =
	Object.fromEntries(
...
const pwmInfo: Record<
	number,
	Record<number, [current: number, force: number]>
> = Object.fromEntries(
	Object.entries(tupledData).map(([k, v]) => [
		k,
...
await writeFile(
	path.join(currentDirectory, '..', 'src', 'data', 'pwm.ts'),
	`${HEADER}export const fromPwmData: Record<number, Record<number, [current: number, force: number]>> = ${JSON.stringify(pwmInfo, null, '\t')};`,
	'utf-8'
);
